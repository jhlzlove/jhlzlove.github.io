...
ES的内部使用了Restful API，对程序员友好；Kibana提供了友好的客户端操作页面，具有相关的语法提示、图表分析等强大的功能。只要掌握了语法皆可以在项目中轻松使用。类似与MySQL和Navicat。
...
- [ElasticSearch + Kibana使用总结](#elasticsearch--kibana使用总结)
  - [ElasticSearch7 分布式搜索引擎](#elasticsearch7-分布式搜索引擎)
  - [ES 和 Kibana 的安装配置](#es-和-kibana-的安装配置)
...
# ElasticSearch + Kibana使用总结
## ElasticSearch7 分布式搜索引擎
...
ES中的默认使用的是标准分词器(StandardAnalyzer)，中文：单字分词；英文：单词分词。
ES中只有 text 类型是分词的，剩下的 keyword integer date 等类型都不分词。

## ES 和 Kibana 的安装配置

注意，kibana必须和elasticsearch同版本。这里以安装的 7.14 版本为例，安装方式都是解压 `.tar.gz` 包的方式，docker 方式安装比较简单。

elasticsearch 默认 web 端口：9200；tcp端口：9300(集群通信)
kibana默认端口：5601
...
设置完成以后可以启动 es 服务；进入 es 的 bin 目录，运行以下命令：
```bash{.line-numbers}
# 直接启动
./elasticsearch
# 后台启动 以守护进程启动
...
```{.line-numbers}
    ...
    *       hard	nproc       4096
```
1. root 用户修改`vim /etc/sysctl.conf ` ，可能你的系统中没有这个文件，没关系，可以手动创建一个，加入内容：`vm.max_map_count=262144`。然后使用 `sysctl -p` 使其生效并查看输出内容是否和自己设置的一样。

### kibana 配置

同样是修改 kibana 安装目录中的 config 下的 `kibana.yml` 文件，修改以下内容：
```yml{.line-numbers}
# 开启远程连接
server.host: 0.0.0.0
elasticsearch.hosts: [es的ip地址:端口号]
```
> Kibana启动之前确保ES服务已经启动。
...
2. 映射（Mapping）：映射是定义一个文档和它所包含的字段如何被存储和索引的过程。在默认配置下，ES可以根据插入的数据自动地创建mapping，也可以手动创建mapping。mapping中主要包括字段名、字段类型等。
...
一般都是在创建索引的时候创建映射，脱离了索引映射也就没有意义了。创建映射时不需要指定类型的长度。
```json{.line-numbers}
// 创建映射，一般在创建索引时手动创建映射
PUT /index_name {
    // 指定分片信息，可以不写，使用默认
...
ES支持的中文分词器有IK、smartCN等，推荐的是IK分词器。
...
@Field(type = FieldType.Keyword) 指定类型

#### 操作

save 方法如果 id 存在时更新，不存在时添加。


### RestHighLevelClient

使用RestHighLevelClient非常简单，会使用Kibana就可以。可能这也是大家喜欢使用的原因吧。
...
查看集群状态：
`http://任意集群节点ip:端口/_cat/health?v`

### Head插件查看ES状态

https://github.com/mobz/elasticsearch-head.git
下载资源包，解压，进入解压目录，安装依赖 `npm install`，启动`npm run start`